...
`,tt=D.p`
  ...
  font-weight: normal;
  padding: 1rem 1.2rem;
`,et=["day","hour","minutes"],wt=({title:r,createdDate:k,diffDate:_,detail:M,editTodo:Y,deleteTodo:L})=>{const[w,S]=B.useState(nt(M).status),b=q(_).format("DD,,mm").split(",").map((f,h)=>+f==0?null:h===0?+f==1?null:`${+f-1} ${et[h]} `:`${+f} ${et[h]} `);return d.jsxs(yt,{children:[d.jsxs(vt,{children:[d.jsx(Dt,{children:r}),d.jsxs(bt,{children:[d.jsx(K,{className:"text-blue-500 hover:text-blue-600",onClick:Y,children:"Edit"}),d.jsx(K,{className:"text-red-500 hover:text-red-600",onClick:L,children:"Delete"})]})]}),d.jsx(Mt,{children:w?d.jsxs(tt,{children:[d.jsxs(X,{children:[k," | ",b.map(f=>f),"ago"]}),nt(M).context,d.jsxs("span",{style:{cursor:"pointer",textAlign:"right"},className:"text-green-300 hover:text-green-600",onClick:()=>S(!1),children:[" ","Show more"]})]}):d.jsxs(tt,{children:[d.jsxs(X,{children:[k," | ",b.map(f=>f),"ago"]}),M]})})]})},St=B.memo(wt);function nt(r){return r?{status:r.trim().length>100,context:r.trim().slice(0,100)+"..."}:{status:!1,context:null}}const jt=U.object({date:U.date().default(()=>new Date),title:U.string().min(1).max(300,{message:"the title can't over the 120 sizes"}).refine(r=>r!=="",{message:"title is required"}),detail:U.string().max(5e3).optional()}),rt=r=>jt.parse(r),Tt=D.div`
  display: flex;
  flex-direction: column;
...
`,_t=()=>{var f;const r=B.useRef(null),k=B.useRef(null),_=dt(h=>h.todo.todoList),Y=(f=lt(mt).currentUser)==null?void 0:f.uid,L=ht(),w=()=>{var v,T;const h=(v=r.current)==null?void 0:v.value,p=(T=k.current)==null?void 0:T.value,j=rt({title:h,detail:p}),g={uid:Y,todoList:[{date:j.date,title:j.title,detail:j.detail}]};L(pt(g)),r.current.value="",k.current.value=""},S=h=>{var T,N;const p=(T=r.current)==null?void 0:T.value,j=(N=k.current)==null?void 0:N.value,g=rt({title:p,detail:j}),v={date:h,title:g.title,detail:g.detail};L($t(v))},b=h=>{L(xt({targetDate:h}))};return d.jsxs(ft,{children:[d.jsx(kt,{children:d.jsxs("h1",{children:["What you going ",d.jsx("span",{className:" font-bold text-blue-500",children:"To Do"})]})}),d.jsxs(Tt,{children:[d.jsx(Lt,{children:_&&_.map(({date:h,title:p,detail:j},g)=>{const v=`currentTodoList-${p}${g}`,T=q(h).format("YY MM.DD HH:mm"),N=q().diff(h);return d.jsx(St,{editTodo:()=>S(h),deleteTodo:()=>b(h),title:p,createdDate:T,diffDate:N,detail:j},v)})}),d.jsxs("div",{className:"flex flex-col mt-2 box-border p-2",children:[d.jsxs("div",{className:"flex",children:[d.jsx("input",{ref:r,className:"border border-gray-400 p-2 rounded flex-grow mr-2 text-slate-900",placeholder:"What do you going to do today?"}),d.jsx("button",{className:"bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded w-1/3",onClick:w,children:"Add"})]}),d.jsx("textarea",{ref:k,className:"border border-gray-400 p-2 rounded w-full text-slate-900 mt-2",placeholder:"details..."})]})]})]})};export{_t as default};
